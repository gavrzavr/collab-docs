...
function createMcpServer(userEmail: string | null = null): McpServer {
  ...
  registerTool(
    "read_document",
    "TRIGGER: When the user pastes ANY URL from postpaper.co (e.g. https://postpaper.co/doc/abc123, https://www.postpaper.co/doc/abc123, https://postpaper.co/v/sometoken) into the chat, call THIS tool with that URL — even if the user doesn't explicitly say 'read'. DO NOT use WebFetch / web_search / browse on postpaper.co URLs: the page is login-gated and returns a sign-in HTML form to anonymous fetchers, while this MCP tool resolves access via your authenticated key (for /doc/:id) or the share token (for /v/:token) and returns the actual document content. Read one page of a PostPaper document — a live, multi-user, block-based editor where each block has a stable ID and is an independent unit of meaning. ALWAYS call this before editing — returns blocks with IDs so you can make surgical edits via update_block / insert_block / delete_block (preferred) instead of rewriting. A document can have multiple pages (Excel-style tabs) — without a page argument you get the first one. Use list_pages to discover other pages; pass their id or title via 'page' to read a specific tab. Core mindset: think in blocks, not pages; one idea per block; headings are navigation, not decoration; preserve collaborators' work — do not touch blocks unrelated to the task. The output also explains how to make INTRA-DOC LINKS — markdown links pointing at specific blocks/tabs that, when clicked in the editor, jump to and highlight the target. Use them instead of phrases like 'see the section above/below' — they're easier to navigate and survive collaborators reordering content.",
    {
